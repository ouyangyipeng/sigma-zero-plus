...
\subsection{The $\sigma$-zero Algorithm}
...
\end{equation}
where $\cL_{\text{adv}}$ is the difference of logits (DL) adversarial loss, and the second term approximates the $\ell_0$-norm.

The algorithm employs three key mechanisms:
\begin{itemize}
    \item \textbf{Dynamic thresholding:} A threshold $\tau$ is adaptively updated during optimization to progressively zero out small perturbation components, enforcing sparsity.
    \item \textbf{Gradient normalization:} Gradients are normalized to ensure stable optimization across different scales.
    \item \textbf{Cosine annealing:} The learning rate follows a cosine schedule for smooth convergence.
\end{itemize}
